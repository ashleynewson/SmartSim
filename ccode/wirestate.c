...
static void _vala_array_add106 (Connection*** array, int* length, int* size, Connection* value);
void wire_state_add_affected (WireState* self, ComponentState* componentState);
static void _vala_array_add107 (ComponentState*** array, int* length, int* size, ComponentState* value);
...
void wire_state_record (WireState* self) {
	...
	if (_tmp4_ == _tmp8_) {
		...
		_tmp14_ = newState;
		_vala_array_add105 (&self->priv->stateHistory, &self->priv->stateHistory_length1, &self->priv->_stateHistory_size_, &_tmp14_);
	}
}
...
void wire_state_add_interface (WireState* self, Connection* connection) {
	...
	if (!_tmp1_) {
		...
		_vala_array_add106 (&self->priv->interfaces, &self->priv->interfaces_length1, &self->priv->_interfaces_size_, _tmp4_);
	}
}
...
void wire_state_add_affected (WireState* self, ComponentState* componentState) {
	...
	_vala_array_add107 (&self->priv->affectedComponents, &self->priv->affectedComponents_length1, &self->priv->_affectedComponents_size_, _tmp2_);
}
...
void wire_state_start_recording (WireState* self, gint unknownTime) {
	...
	_tmp5__length1 = self->priv->stateHistory_length1;
	_tmp6_ = unknownState;
	_vala_array_add108 (&self->priv->stateHistory, &self->priv->stateHistory_length1, &self->priv->_stateHistory_size_, &_tmp6_);
	_tmp7_ = self->priv->stateHistory;
	_tmp7__length1 = self->priv->stateHistory_length1;
	_tmp8_ = initialState;
	_vala_array_add109 (&self->priv->stateHistory, &self->priv->stateHistory_length1, &self->priv->_stateHistory_size_, &_tmp8_);
