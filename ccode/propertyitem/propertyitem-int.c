/* propertyitem-int.c generated by valac 0.16.0, the Vala compiler
...
gint property_item_int_get_data_throw (PropertySet* propertySet, const gchar* name, GError** error) {
	...
	if (_tmp3_ != NULL) {
		PropertyItem* _tmp4_;
		_tmp4_ = propertyItem;
		if (IS_PROPERTY_ITEM_INT (_tmp4_)) {
			PropertyItem* _tmp5_;
			gint _tmp6_;
			_tmp5_ = propertyItem;
			_tmp6_ = (IS_PROPERTY_ITEM_INT (_tmp5_) ? ((PropertyItemInt*) _tmp5_) : NULL)->data;
			result = _tmp6_;
...
void property_item_int_set_data_throw (PropertySet* propertySet, const gchar* name, gint data, GError** error) {
	...
	if (_tmp3_ != NULL) {
		...
		if (IS_PROPERTY_ITEM_INT (_tmp4_)) {
			...
			_tmp5_ = propertyItem;
			_tmp6_ = data;
			(IS_PROPERTY_ITEM_INT (_tmp5_) ? ((PropertyItemInt*) _tmp5_) : NULL)->data = _tmp6_;
			_property_item_unref0 (propertyItem);
			return;
		}
	}
...
static void property_item_int_real_read_widget (PropertyItem* base, GtkWidget* propertyWidget) {
	...
	if (_tmp0_ != NULL) {
		...
		if (GTK_IS_SPIN_BUTTON (_tmp1_)) {
			...
			_tmp3_ = gtk_spin_button_get_value_as_int (GTK_IS_SPIN_BUTTON (_tmp2_) ? ((GtkSpinButton*) _tmp2_) : NULL);
			self->data = _tmp3_;
		}
	}
}
