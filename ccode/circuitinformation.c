/* circuitinformation.c generated by valac 0.16.0, the Vala compiler
...
static void _vala_array_add77 (CircuitInformationComponentCount** array, int* length, int* size, const CircuitInformationComponentCount* value);
...
CircuitInformation* circuit_information_construct (GType object_type, Project* project) {
	...
	if (_tmp4_ != NULL) {
		...
		gchar* _tmp34_;
		gchar* _tmp35_;
		CircuitInformationComponentCount* _tmp36_;
...
" dependencies.", NULL);
		...
		_tmp31_ = project;
		_tmp32_ = _tmp31_->rootComponent;
		component_def_create_information ((ComponentDef*) _tmp32_, self);
		_tmp33_ = self->priv->_summary;
		_tmp34_ = g_strconcat (_tmp33_, "Total Component Counts:\n", NULL);
		_tmp35_ = _tmp34_;
		circuit_information_set_summary (self, _tmp35_);
		_g_free0 (_tmp35_);
		_tmp36_ = self->priv->componentCounts;
		_tmp36__length1 = self->priv->componentCounts_length1;
		{
			...
			componentCount_collection = _tmp36_;
			componentCount_collection_length1 = _tmp36__length1;
			for (componentCount_it = 0; componentCount_it < _tmp36__length1; componentCount_it = componentCount_it + 1) {
				CircuitInformationComponentCount _tmp37_ = {0};
				CircuitInformationComponentCount componentCount = {0};
				circuit_information_component_count_copy (&componentCount_collection[componentCount_it], &_tmp37_);
				componentCount = _tmp37_;
				{
					const gchar* _tmp38_;
					CircuitInformationComponentCount _tmp39_;
					ComponentDef* _tmp40_;
					const gchar* _tmp41_;
					gchar* _tmp42_;
					gchar* _tmp43_;
					gchar* _tmp44_;
					gchar* _tmp45_;
					CircuitInformationComponentCount _tmp46_;
					gint _tmp47_;
					gchar* _tmp48_ = NULL;
					gchar* _tmp49_;
					gchar* _tmp50_;
					...
					gchar* _tmp54_;
					gchar* _tmp55_;
					_tmp38_ = self->priv->_summary;
					_tmp39_ = componentCount;
					_tmp40_ = _tmp39_.componentDef;
					_tmp41_ = _tmp40_->name;
					_tmp42_ = g_strconcat ("  ", _tmp41_, NULL);
					_tmp43_ = _tmp42_;
					_tmp44_ = g_strconcat (_tmp43_, ": ", NULL);
					_tmp45_ = _tmp44_;
					_tmp46_ = componentCount;
					_tmp47_ = _tmp46_.count;
					...
					_tmp55_ = _tmp54_;
					circuit_information_set_summary (self, _tmp55_);
					_g_free0 (_tmp55_);
					_g_free0 (_tmp53_);
					_g_free0 (_tmp51_);
					_g_free0 (_tmp49_);
		...
		_tmp56_ = (GtkMessageDialog*) gtk_message_dialog_new (NULL, GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, "You cannot generate circuit statistics without a root component.");
		_tmp57_ = g_object_ref_sink (_tmp56_);
		messageDialog = _tmp57_;
		_tmp58_ = messageDialog;
		gtk_dialog_run ((GtkDialog*) _tmp58_);
		_tmp59_ = messageDialog;
...
void circuit_information_count_component (CircuitInformation* self, ComponentDef* componentDef) {
	...
	if (_tmp13_ == _tmp14__length1) {
		...
		_vala_array_add77 (&self->priv->componentCounts, &self->priv->componentCounts_length1, &self->priv->_componentCounts_size_, &_tmp19_);
		circuit_information_component_count_destroy (&componentCount);
	}
}
