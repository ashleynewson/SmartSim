...
void image_exporter_export_png (ImageExporterRenderer renderer, void* renderer_target) {
	...
	if (_tmp14_ == ((gint) GTK_RESPONSE_APPLY)) {
		...
		gtk_buildable_set_name ((GtkBuildable*) _tmp16_, "Portable Network Graphic (.png)");
		_tmp17_ = pngFileFilter;
		gtk_file_filter_add_pattern (_tmp17_, "*.png");
		_tmp18_ = (GtkFileChooserDialog*) gtk_file_chooser_dialog_new ("Export to PNG", NULL, GTK_FILE_CHOOSER_ACTION_SAVE, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_SAVE, GTK_RESPONSE_ACCEPT, NULL);
		g_object_ref_sink (_tmp18_);
		fileChooser = _tmp18_;
		_tmp19_ = fileChooser;
		_tmp20_ = pngFileFilter;
		...
		while (TRUE) {
			...
			if (_tmp24_ == ((gint) GTK_RESPONSE_ACCEPT)) {
				...
				if (_tmp28_) {
					...
					_tmp32_ = _tmp31_;
					_tmp33_ = pngFileFilter;
					if (_tmp32_ == _tmp33_) {
						const gchar* _tmp34_;
						gchar* _tmp35_;
						_tmp34_ = filename;
						_tmp35_ = g_strconcat (_tmp34_, ".png", NULL);
						_g_free0 (filename);
						filename = _tmp35_;
					}
				}
				...
				if (_tmp37_) {
					...
					_tmp38_ = fileChooser;
					_tmp39_ = filename;
					_tmp40_ = basic_dialog_ask_overwrite ((GtkWindow*) _tmp38_, _tmp39_);
					if (_tmp40_ == ((gint) GTK_RESPONSE_YES)) {
						stillChoosing = FALSE;
					}
				} else {
					stillChoosing = FALSE;
	...
	if (g_strcmp0 (_tmp43_, "") != 0) {
		...
		gdouble _tmp54_;
		_tmp44_ = propertySet;
		_tmp45_ = property_item_selection_get_data (_tmp44_, "Format");
		_tmp46_ = _tmp45_;
		_tmp48_ = (NULL == _tmp46_) ? 0 : g_quark_from_string (_tmp46_);
		g_free (_tmp46_);
		if (_tmp48_ == ((0 != _tmp47_label0) ? _tmp47_label0 : (_tmp47_label0 = g_quark_from_static_string ("RGB (White Background)")))) {
			switch (0) {
				default:
				{
					imageFormat = IMAGE_EXPORTER_IMAGE_FORMAT_PNG_RGB;
		...
		} else if (_tmp48_ == ((0 != _tmp47_label1) ? _tmp47_label1 : (_tmp47_label1 = g_quark_from_static_string ("ARGB (Transparent Background)")))) {
			switch (0) {
				default:
				{
		...
		_tmp49_ = propertySet;
		_tmp50_ = property_item_double_get_data (_tmp49_, "Resolution");
		resolution = _tmp50_;
		_tmp51_ = renderer;
		...
		_tmp53_ = imageFormat;
		_tmp54_ = resolution;
		_tmp51_ (_tmp52_, _tmp53_, _tmp54_, _tmp51__target);
	}
	_properties_query_unref0 (propertiesQuery);
	_property_item_unref0 (resolutionProperty);
	_property_item_unref0 (formatProperty);
...
void image_exporter_export_pdf (ImageExporterRenderer renderer, void* renderer_target) {
	...
	gtk_buildable_set_name ((GtkBuildable*) _tmp2_, "Portable Document Format (.pdf)");
	_tmp3_ = pdfFileFilter;
	gtk_file_filter_add_pattern (_tmp3_, "*.pdf");
	_tmp4_ = (GtkFileChooserDialog*) gtk_file_chooser_dialog_new ("Export to PDF", NULL, GTK_FILE_CHOOSER_ACTION_SAVE, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_SAVE, GTK_RESPONSE_ACCEPT, NULL);
	g_object_ref_sink (_tmp4_);
	fileChooser = _tmp4_;
	_tmp5_ = fileChooser;
	_tmp6_ = pdfFileFilter;
	...
	while (TRUE) {
		...
		if (_tmp10_ == ((gint) GTK_RESPONSE_ACCEPT)) {
			...
			if (_tmp14_) {
				FILE* _tmp15_;
				_tmp15_ = stdout;
				fprintf (_tmp15_, "File extension already given\n");
			} else {
				...
				_tmp16_ = fileChooser;
				_tmp17_ = gtk_file_chooser_get_filter ((GtkFileChooser*) _tmp16_);
				_tmp18_ = _tmp17_;
				_tmp19_ = pdfFileFilter;
				if (_tmp18_ == _tmp19_) {
					const gchar* _tmp20_;
					gchar* _tmp21_;
					_tmp20_ = filename;
			...
			if (_tmp23_) {
				...
				_tmp24_ = fileChooser;
				_tmp25_ = filename;
				_tmp26_ = basic_dialog_ask_overwrite ((GtkWindow*) _tmp24_, _tmp25_);
				if (_tmp26_ == ((gint) GTK_RESPONSE_YES)) {
					stillChoosing = FALSE;
				}
			} else {
				stillChoosing = FALSE;
	...
	if (g_strcmp0 (_tmp29_, "") != 0) {
		ImageExporterRenderer _tmp30_;
		void* _tmp30__target;
		const gchar* _tmp31_;
		_tmp30_ = renderer;
		_tmp30__target = renderer_target;
		_tmp31_ = filename;
		_tmp30_ (_tmp31_, IMAGE_EXPORTER_IMAGE_FORMAT_PDF, (gdouble) 1, _tmp30__target);
	}
	_g_object_unref0 (fileChooser);
	_g_object_unref0 (pdfFileFilter);
	_g_free0 (filename);
...
void image_exporter_export_svg (ImageExporterRenderer renderer, void* renderer_target) {
	...
	if (_tmp11_ == ((gint) GTK_RESPONSE_APPLY)) {
		...
		gtk_buildable_set_name ((GtkBuildable*) _tmp13_, "Scalable Vector Graphic (.svg)");
		_tmp14_ = svgFileFilter;
		gtk_file_filter_add_pattern (_tmp14_, "*.svg");
		_tmp15_ = (GtkFileChooserDialog*) gtk_file_chooser_dialog_new ("Export to SVG", NULL, GTK_FILE_CHOOSER_ACTION_SAVE, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_SAVE, GTK_RESPONSE_ACCEPT, NULL);
		g_object_ref_sink (_tmp15_);
		fileChooser = _tmp15_;
		_tmp16_ = fileChooser;
		_tmp17_ = svgFileFilter;
		...
		while (TRUE) {
			...
			if (_tmp21_ == ((gint) GTK_RESPONSE_ACCEPT)) {
				...
				if (_tmp25_) {
					...
					_tmp29_ = _tmp28_;
					_tmp30_ = svgFileFilter;
					if (_tmp29_ == _tmp30_) {
						const gchar* _tmp31_;
						gchar* _tmp32_;
						_tmp31_ = filename;
						_tmp32_ = g_strconcat (_tmp31_, ".svg", NULL);
						_g_free0 (filename);
						filename = _tmp32_;
					}
				}
				...
				if (_tmp34_) {
					...
					_tmp35_ = fileChooser;
					_tmp36_ = filename;
					_tmp37_ = basic_dialog_ask_overwrite ((GtkWindow*) _tmp35_, _tmp36_);
	...
	if (g_strcmp0 (_tmp40_, "") != 0) {
		...
		ImageExporterRenderer _tmp46_;
		void* _tmp46__target;
		const gchar* _tmp47_;
		ImageExporterImageFormat _tmp48_;
		_tmp41_ = propertySet;
		_tmp42_ = property_item_selection_get_data (_tmp41_, "Background");
		_tmp43_ = _tmp42_;
		_tmp45_ = (NULL == _tmp43_) ? 0 : g_quark_from_string (_tmp43_);
		g_free (_tmp43_);
		if (_tmp45_ == ((0 != _tmp44_label0) ? _tmp44_label0 : (_tmp44_label0 = g_quark_from_static_string ("White Background")))) {
			switch (0) {
				default:
				{
					imageFormat = IMAGE_EXPORTER_IMAGE_FORMAT_SVG;
		...
		} else if (_tmp45_ == ((0 != _tmp44_label1) ? _tmp44_label1 : (_tmp44_label1 = g_quark_from_static_string ("Transparent Background")))) {
			switch (0) {
				default:
				{
		...
		_tmp46_ = renderer;
		_tmp46__target = renderer_target;
		_tmp47_ = filename;
		_tmp48_ = imageFormat;
		_tmp46_ (_tmp47_, _tmp48_, (gdouble) 1, _tmp46__target);
	}
	_properties_query_unref0 (propertiesQuery);
	_property_item_unref0 (formatProperty);
	_property_item_unref0 (propertySet);
