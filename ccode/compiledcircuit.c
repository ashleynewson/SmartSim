		...
		for (componentState_it = 0; componentState_it < _tmp6__length1; componentState_it = componentState_it + 1) {
			...
			componentState = _tmp7_;
			{
				...
				if (_tmp10_) {
					...
					_vala_array_add20 (&permanentComponentStates, &permanentComponentStates_length1, &_permanentComponentStates_size_, _tmp13_);
				}
...
WireState* compiled_circuit_compile_wire (CompiledCircuit* self, WireInst* wireInst, Connection** connections, int connections_length1, ComponentInst** ancestry, int ancestry_length1) {
	...
	_vala_array_add21 (&self->priv->wireStates, &self->priv->wireStates_length1, &self->priv->_wireStates_size_, _tmp52_);
	result = wireState;
	return result;
}
...
void compiled_circuit_add_component (CompiledCircuit* self, ComponentState* componentState) {
	...
	_vala_array_add22 (&self->priv->componentStates, &self->priv->componentStates_length1, &self->priv->_componentStates_size_, _tmp5_);
}
...
void compiled_circuit_add_watch (CompiledCircuit* self, WireState* wireState) {
	...
	if (!_vala_wire_state_array_contains (_tmp1_, _tmp1__length1, _tmp0_)) {
		...
		_vala_array_add23 (&self->priv->watchedWireStates, &self->priv->watchedWireStates_length1, &self->priv->_watchedWireStates_size_, _tmp4_);
	}
}
		...
		for (watchedWireState_it = 0; watchedWireState_it < _tmp1__length1; watchedWireState_it = watchedWireState_it + 1) {
			...
			watchedWireState = _tmp2_;
			{
				...
				if (_tmp3_ != _tmp4_) {
					...
					_vala_array_add24 (&newWatchedWireStates, &newWatchedWireStates_length1, &_newWatchedWireStates_size_, _tmp7_);
				}
...
gint compiled_circuit_expand_component (CompiledCircuit* self, gint x, gint y) {
	...
	if (_tmp4_ != NULL) {
		...
		if (G_TYPE_CHECK_INSTANCE_TYPE (_tmp6_, TYPE_CUSTOM_COMPONENT_DEF)) {
			...
			_vala_array_add25 (&self->priv->viewedAncestry, &self->priv->viewedAncestry_length1, &self->priv->_viewedAncestry_size_, _tmp9_);
			compiled_circuit_update_displayed (self);
			result = 0;
			...
			for (componentState_it = 0; componentState_it < _tmp10__length1; componentState_it = componentState_it + 1) {
				...
				componentState = _tmp11_;
				{
					...
					if (_tmp14_ == 1) {
						...
						_vala_array_add26 (&displayedComponentStates, &displayedComponentStates_length1, &_displayedComponentStates_size_, _tmp20_);
					} else {
			...
			for (wireState_it = 0; wireState_it < _tmp22__length1; wireState_it = wireState_it + 1) {
				...
				wireState = _tmp23_;
				{
					...
					if (_tmp26_ == 1) {
						...
						_vala_array_add27 (&displayedWireStates, &displayedWireStates_length1, &_displayedWireStates_size_, _tmp32_);
					} else {
						WireState* _tmp33_;
						_tmp33_ = wireState;
