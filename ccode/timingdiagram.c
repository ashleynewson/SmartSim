...
static void _vala_array_add106 (WireState*** array, int* length, int* size, WireState* value);
...
static void _vala_array_add109 (WireState*** array, int* length, int* size, WireState* value);
static void _vala_array_add110 (gchar*** array, int* length, int* size, gchar* value);
...
static void _vala_array_add111 (WireState*** array, int* length, int* size, WireState* value);
...
void timing_diagram_add_wire (TimingDiagram* self, WireState* newWireState) {
	...
	if (_tmp11_ == ((gint) GTK_RESPONSE_APPLY)) {
		...
		_vala_array_add106 (&self->priv->wireStates, &self->priv->wireStates_length1, &self->priv->_wireStates_size_, _tmp17_);
		_tmp18_ = label;
		if (g_strcmp0 (_tmp18_, "") == 0) {
			...
			_vala_array_add107 (&self->priv->labels, &self->priv->labels_length1, &self->priv->_labels_size_, _tmp23_);
			_g_free0 (_tmp22_);
		} else {
			gchar** _tmp24_;
			...
			_vala_array_add108 (&self->priv->labels, &self->priv->labels_length1, &self->priv->_labels_size_, _tmp26_);
		}
		...
		i = 0;
		{
			...
			while (TRUE) {
				...
				if (_tmp7_ != _tmp8_) {
					...
					_vala_array_add109 (&newWireStates, &newWireStates_length1, &_newWireStates_size_, _tmp13_);
					_tmp14_ = newLabels;
					...
					_vala_array_add110 (&newLabels, &newLabels_length1, &_newLabels_size_, _tmp18_);
				} else {
					WireState** _tmp19_;
					gint _tmp19__length1;
					gint _tmp20_;
...
static void timing_diagram_move_wire (TimingDiagram* self, gint fromNumber, gint toNumber) {
	...
	if (_tmp11_) {
			...
			i = 0;
			{
				...
				while (TRUE) {
					...
					if (_tmp27_ == _tmp28_) {
						...
						_vala_array_add111 (&newWireStates, &newWireStates_length1, &_newWireStates_size_, _tmp31_);
						_tmp32_ = newLabels;
						_tmp32__length1 = newLabels_length1;
						_tmp33_ = movingLabel;
						_tmp34_ = g_strdup (_tmp33_);
						_vala_array_add112 (&newLabels, &newLabels_length1, &_newLabels_size_, _tmp34_);
						_tmp35_ = insertCount;
					...
					if (_tmp36_ != _tmp37_) {
						...
						_vala_array_add113 (&newWireStates, &newWireStates_length1, &_newWireStates_size_, _tmp42_);
						_tmp43_ = newLabels;
						...
						_vala_array_add114 (&newLabels, &newLabels_length1, &_newLabels_size_, _tmp47_);
						_tmp48_ = insertCount;
		...
		if (_tmp49_ == _tmp50_) {
			...
			_vala_array_add115 (&newWireStates, &newWireStates_length1, &_newWireStates_size_, _tmp53_);
			_tmp54_ = newLabels;
			_tmp54__length1 = newLabels_length1;
			_tmp55_ = movingLabel;
			_tmp56_ = g_strdup (_tmp55_);
			_vala_array_add116 (&newLabels, &newLabels_length1, &_newLabels_size_, _tmp56_);
			_tmp57_ = insertCount;
