/* plugincomponentmanager.c generated by valac 0.18.1, the Vala compiler
...
static void _vala_array_add58 (PluginComponentManager*** array, int* length, int* size, PluginComponentManager* value);
...
static void plugin_component_manager_register (PluginComponentManager* pluginComponentManager) {
	...
	_vala_array_add58 (&plugin_component_manager_pluginComponentManagers, &plugin_component_manager_pluginComponentManagers_length1, &_plugin_component_manager_pluginComponentManagers_size_, _tmp3_);
	_tmp4_ = stdout;
...
"component menu.", NULL);
	...
	if (_tmp112_ == NULL) {
		...
		_tmp115_ = _tmp114_;
		_tmp116_ = g_strconcat (PACKAGE_DATADIR "plugins/", _tmp115_, NULL);
		_g_free0 (libraryPath);
		...
		gchar* _tmp122_ = NULL;
		_tmp120_ = self->project;
		_tmp121_ = libraryPath;
		_tmp122_ = project_relative_filename (_tmp120_, _tmp121_);
		_g_free0 (libraryPath);
		libraryPath = _tmp122_;
		{
			const gchar* _tmp123_;
			_tmp123_ = libraryPath;
			plugin_component_manager_load_library (self, _tmp123_, &_inner_error_);
			if (_inner_error_ != NULL) {
				if (_inner_error_->domain == PLUGIN_COMPONENT_DEF_LOAD_ERROR) {
		...
		__catch52_plugin_component_def_load_error:
		{
			...
			_inner_error_ = NULL;
			_tmp124_ = _error_;
			if (g_error_matches (_tmp124_, PLUGIN_COMPONENT_DEF_LOAD_ERROR, PLUGIN_COMPONENT_DEF_LOAD_ERROR_LIBRARY_NOT_ACCESSIBLE)) {
				{
					...
					_tmp127_ = _tmp126_;
					_tmp128_ = g_strconcat (PACKAGE_LIBDIR, _tmp127_, NULL);
					_g_free0 (libraryPath);
...
static void plugin_component_manager_load_library (PluginComponentManager* self, const gchar* libraryPath, GError** error) {
	...
	_g_free0 (_tmp5_);
	_g_free0 (_tmp2_);
	fullLibraryPath = _tmp7_;
	_tmp8_ = stdout;
	_tmp9_ = fullLibraryPath;
	fprintf (_tmp8_, "Attempting to open module: %s\n", _tmp9_);
	_tmp10_ = fullLibraryPath;
	_tmp11_ = g_module_open (_tmp10_, 0);
	...
	if (_tmp12_ == NULL) {
		...
		fprintf (_tmp13_, "Unable to open module: %s\n", _tmp14_);
		_tmp15_ = self->filename;
		_tmp16_ = g_strconcat ("Library could not be opened: \"", _tmp15_, NULL);
		_tmp17_ = _tmp16_;
		_tmp18_ = g_strconcat (_tmp17_, "\": \"", NULL);
		_tmp19_ = _tmp18_;
		_tmp20_ = fullLibraryPath;
		_tmp21_ = g_strconcat (_tmp19_, _tmp20_, NULL);
		_tmp22_ = _tmp21_;
		_tmp23_ = g_strconcat (_tmp22_, "\": ", NULL);
		_tmp24_ = _tmp23_;
		_tmp25_ = g_error_new_literal (PLUGIN_COMPONENT_DEF_LOAD_ERROR, PLUGIN_COMPONENT_DEF_LOAD_ERROR_LIBRARY_NOT_ACCESSIBLE, _tmp24_);
		_tmp26_ = _tmp25_;
		_g_free0 (_tmp24_);
		_g_free0 (_tmp22_);
		_g_free0 (_tmp19_);
	...
	_tmp27_ = stdout;
	fprintf (_tmp27_, "Successfully opened module.\n");
	init_pointer = NULL;
	...
	_tmp28_ = self->module;
	_tmp30_ = g_module_symbol (_tmp28_, "plugin_component_init", &_tmp29_);
	init_pointer = _tmp29_;
	if (_tmp30_) {
		void* _tmp31_;
		_tmp31_ = init_pointer;
		if (_tmp31_ != NULL) {
			FILE* _tmp32_;
			void* _tmp33_;
			PluginComponentManagerinit_delegate _tmp34_;
			void* _tmp34__target;
			gboolean _tmp35_ = FALSE;
			_tmp32_ = stdout;
			fprintf (_tmp32_, "Initialising plugin... (plugin_component_init).\n");
			_tmp33_ = init_pointer;
			(init_function_target_destroy_notify == NULL) ? NULL : (init_function_target_destroy_notify (init_function_target), NULL);
			init_function = NULL;
			init_function_target = NULL;
			init_function_target_destroy_notify = NULL;
			init_function = (PluginComponentManagerinit_delegate) _tmp33_;
			init_function_target = NULL;
			init_function_target_destroy_notify = NULL;
			_tmp34_ = init_function;
			_tmp34__target = init_function_target;
			_tmp35_ = _tmp34_ (self, _tmp34__target);
			if (_tmp35_ == FALSE) {
				FILE* _tmp36_;
				const gchar* _tmp37_;
				gchar* _tmp38_;
				...
				GError* _tmp48_;
				_tmp36_ = stdout;
				fprintf (_tmp36_, "Error initialising plugin: Plugin init function reported failure.\n");
				_tmp37_ = self->filename;
				_tmp38_ = g_strconcat ("Plugin init function reported failure: \"", _tmp37_, NULL);
				_tmp39_ = _tmp38_;
				_tmp40_ = g_strconcat (_tmp39_, "\": \"", NULL);
				_tmp41_ = _tmp40_;
				_tmp42_ = fullLibraryPath;
				_tmp43_ = g_strconcat (_tmp41_, _tmp42_, NULL);
				_tmp44_ = _tmp43_;
				_tmp45_ = g_strconcat (_tmp44_, "\": ", NULL);
				_tmp46_ = _tmp45_;
				_tmp47_ = g_error_new_literal (PLUGIN_COMPONENT_DEF_LOAD_ERROR, PLUGIN_COMPONENT_DEF_LOAD_ERROR_INIT_ERROR, _tmp46_);
				_tmp48_ = _tmp47_;
				_g_free0 (_tmp46_);
				_g_free0 (_tmp44_);
				_g_free0 (_tmp41_);
				...
				FILE* _tmp49_;
				_tmp49_ = stdout;
				fprintf (_tmp49_, "Initialising plugin... (plugin_component_init).\n");
			}
		} else {
			FILE* _tmp50_;
			_tmp50_ = stdout;
			fprintf (_tmp50_, "Got null plugin_component_init function.\n");
		}
	}
	_tmp51_ = init_pointer;
	if (_tmp51_ == NULL) {
		FILE* _tmp52_;
		_tmp52_ = stdout;
		fprintf (_tmp52_, "Plugin has no init function (plugin_component_init).\n");
	}
	_tmp53_ = self->module;
	_tmp55_ = g_module_symbol (_tmp53_, "plugin_component_get_def", &_tmp54_);
	get_def_pointer = _tmp54_;
	if (_tmp55_) {
		void* _tmp56_;
		...
		if (_tmp56_ != NULL) {
			...
			get_def_function_target = NULL;
			get_def_function_target_destroy_notify = NULL;
			get_def_function = (PluginComponentManagerget_def_delegate) _tmp57_;
			get_def_function_target = NULL;
			get_def_function_target_destroy_notify = NULL;
		}
	}
	_tmp58_ = get_def_pointer;
	if (_tmp58_ == NULL) {
		FILE* _tmp59_;
		const gchar* _tmp60_;
		gchar* _tmp61_;
		...
		GError* _tmp71_;
		_tmp59_ = stdout;
		fprintf (_tmp59_, "Error initialising plugin: Could not get component definition function" \
".\n");
		_tmp60_ = self->filename;
		_tmp61_ = g_strconcat ("Could not get component definition function: \"", _tmp60_, NULL);
		_tmp62_ = _tmp61_;
		_tmp63_ = g_strconcat (_tmp62_, "\": \"", NULL);
		_tmp64_ = _tmp63_;
		_tmp65_ = fullLibraryPath;
		_tmp66_ = g_strconcat (_tmp64_, _tmp65_, NULL);
		_tmp67_ = _tmp66_;
		_tmp68_ = g_strconcat (_tmp67_, "\": ", NULL);
		_tmp69_ = _tmp68_;
		_tmp70_ = g_error_new_literal (PLUGIN_COMPONENT_DEF_LOAD_ERROR, PLUGIN_COMPONENT_DEF_LOAD_ERROR_LIBRARY_NOT_COMPATIBLE, _tmp69_);
		_tmp71_ = _tmp70_;
		_g_free0 (_tmp69_);
		_g_free0 (_tmp67_);
		_g_free0 (_tmp64_);
		...
		PluginComponentManagerget_def_delegate _tmp72_;
		void* _tmp72__target;
		const gchar* _tmp73_;
		PluginComponentDef* _tmp74_ = NULL;
		PluginComponentDef* _tmp75_;
		_tmp72_ = get_def_function;
		_tmp72__target = get_def_function_target;
		_tmp73_ = self->filename;
		_tmp74_ = _tmp72_ (_tmp73_, _tmp72__target);
		_component_def_unref0 (self->pluginComponentDef);
		self->pluginComponentDef = _tmp74_;
		_tmp75_ = self->pluginComponentDef;
		if (_tmp75_ == NULL) {
			FILE* _tmp76_;
			const gchar* _tmp77_;
			gchar* _tmp78_;
			...
			GError* _tmp88_;
			_tmp76_ = stdout;
			fprintf (_tmp76_, "Error initialising plugin: Failure getting component definition.\n");
			_tmp77_ = self->filename;
			_tmp78_ = g_strconcat ("Failure getting component definition: \"", _tmp77_, NULL);
			_tmp79_ = _tmp78_;
			_tmp80_ = g_strconcat (_tmp79_, "\": \"", NULL);
			_tmp81_ = _tmp80_;
			_tmp82_ = fullLibraryPath;
			_tmp83_ = g_strconcat (_tmp81_, _tmp82_, NULL);
			_tmp84_ = _tmp83_;
			_tmp85_ = g_strconcat (_tmp84_, "\": ", NULL);
			_tmp86_ = _tmp85_;
			_tmp87_ = g_error_new_literal (PLUGIN_COMPONENT_DEF_LOAD_ERROR, PLUGIN_COMPONENT_DEF_LOAD_ERROR_INIT_ERROR, _tmp86_);
			_tmp88_ = _tmp87_;
			_g_free0 (_tmp86_);
			_g_free0 (_tmp84_);
			_g_free0 (_tmp81_);
