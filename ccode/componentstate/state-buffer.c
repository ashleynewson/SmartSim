/* state-buffer.c generated by valac 0.16.0, the Vala compiler
 * generated from state-buffer.vala, do not modify */
...
typedef struct _BufferComponentStatePrivate BufferComponentStatePrivate;
...
struct _BufferComponentState {
	ComponentState parent_instance;
	BufferComponentStatePrivate * priv;
};

struct _BufferComponentStateClass {
	ComponentStateClass parent_class;
};

struct _BufferComponentStatePrivate {
	Connection* inputWire;
	Connection* outputWire;
};


static gpointer buffer_component_state_parent_class = NULL;
...
#define BUFFER_COMPONENT_STATE_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), TYPE_BUFFER_COMPONENT_STATE, BufferComponentStatePrivate))
enum  {
	BUFFER_COMPONENT_STATE_DUMMY_PROPERTY
};
...
static ComponentInst** _vala_array_dup73 (ComponentInst** self, int length);
static void buffer_component_state_real_update (ComponentState* base);
gboolean connection_get_signalState (Connection* self);
void connection_set_signalState (Connection* self, gboolean value);
static void buffer_component_state_finalize (ComponentState* obj);
...
BufferComponentState* buffer_component_state_construct (GType object_type, Connection* inputWire, Connection* outputWire, ComponentInst** ancestry, int ancestry_length1, ComponentInst* componentInst) {
	BufferComponentState* self = NULL;
	Connection* _tmp0_;
	Connection* _tmp1_;
	Connection* _tmp2_;
	Connection* _tmp3_;
	...
	self = (BufferComponentState*) component_state_construct (object_type);
	_tmp0_ = inputWire;
	...
	self->priv->outputWire = _tmp4_;
	_tmp5_ = ancestry;
	_tmp5__length1 = ancestry_length1;
	_tmp6_ = (_tmp5_ != NULL) ? _vala_array_dup73 (_tmp5_, _tmp5__length1) : ((gpointer) _tmp5_);
...
BufferComponentState* buffer_component_state_new (Connection* inputWire, Connection* outputWire, ComponentInst** ancestry, int ancestry_length1, ComponentInst* componentInst) {
	return buffer_component_state_construct (TYPE_BUFFER_COMPONENT_STATE, inputWire, outputWire, ancestry, ancestry_length1, componentInst);
}


static void buffer_component_state_real_update (ComponentState* base) {
	BufferComponentState * self;
	gboolean output = FALSE;
	Connection* _tmp0_;
	gboolean _tmp1_;
	gboolean _tmp2_;
	Connection* _tmp3_;
	self = (BufferComponentState*) base;
	_tmp0_ = self->priv->inputWire;
	_tmp1_ = connection_get_signalState (_tmp0_);
	_tmp2_ = _tmp1_;
	output = _tmp2_;
	_tmp3_ = self->priv->outputWire;
	connection_set_signalState (_tmp3_, output);
}


static void buffer_component_state_class_init (BufferComponentStateClass * klass) {
	buffer_component_state_parent_class = g_type_class_peek_parent (klass);
	COMPONENT_STATE_CLASS (klass)->finalize = buffer_component_state_finalize;
	g_type_class_add_private (klass, sizeof (BufferComponentStatePrivate));
	COMPONENT_STATE_CLASS (klass)->update = buffer_component_state_real_update;
}


static void buffer_component_state_instance_init (BufferComponentState * self) {
	self->priv = BUFFER_COMPONENT_STATE_GET_PRIVATE (self);
}


static void buffer_component_state_finalize (ComponentState* obj) {
	BufferComponentState * self;
	self = BUFFER_COMPONENT_STATE (obj);
	_connection_unref0 (self->priv->inputWire);
	_connection_unref0 (self->priv->outputWire);
	COMPONENT_STATE_CLASS (buffer_component_state_parent_class)->finalize (obj);
}


GType buffer_component_state_get_type (void) {
	static volatile gsize buffer_component_state_type_id__volatile = 0;
	if (g_once_init_enter (&buffer_component_state_type_id__volatile)) {
		static const GTypeInfo g_define_type_info = { sizeof (BufferComponentStateClass), (GBaseInitFunc) NULL, (GBaseFinalizeFunc) NULL, (GClassInitFunc) buffer_component_state_class_init, (GClassFinalizeFunc) NULL, NULL, sizeof (BufferComponentState), 0, (GInstanceInitFunc) buffer_component_state_instance_init, NULL };
		GType buffer_component_state_type_id;
		buffer_component_state_type_id = g_type_register_static (TYPE_COMPONENT_STATE, "BufferComponentState", &g_define_type_info, 0);
		g_once_init_leave (&buffer_component_state_type_id__volatile, buffer_component_state_type_id);
	}
	return buffer_component_state_type_id__volatile;
}
