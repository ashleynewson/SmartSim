/* state-xor.c generated by valac 0.16.0, the Vala compiler
 * generated from state-xor.vala, do not modify */
...
typedef struct _XorComponentStatePrivate XorComponentStatePrivate;
...
struct _XorComponentState {
	ComponentState parent_instance;
	XorComponentStatePrivate * priv;
};

struct _XorComponentStateClass {
	ComponentStateClass parent_class;
};

struct _XorComponentStatePrivate {
...
static gpointer xor_component_state_parent_class = NULL;
...
#define XOR_COMPONENT_STATE_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), TYPE_XOR_COMPONENT_STATE, XorComponentStatePrivate))
enum  {
	XOR_COMPONENT_STATE_DUMMY_PROPERTY
};
...
static Connection** _vala_array_dup89 (Connection** self, int length);
void connection_set_affects (Connection* self, ComponentState* componentState);
static ComponentInst** _vala_array_dup90 (ComponentInst** self, int length);
...
static void xor_component_state_finalize (ComponentState* obj);
...
XorComponentState* xor_component_state_construct (GType object_type, Connection** inputWires, int inputWires_length1, Connection* outputWire, ComponentInst** ancestry, int ancestry_length1, ComponentInst* componentInst) {
	...
	_tmp1_ = (_tmp0_ != NULL) ? _vala_array_dup89 (_tmp0_, _tmp0__length1) : ((gpointer) _tmp0_);
	...
	_tmp8_ = (_tmp7_ != NULL) ? _vala_array_dup90 (_tmp7_, _tmp7__length1) : ((gpointer) _tmp7_);
...
XorComponentState* xor_component_state_new (Connection** inputWires, int inputWires_length1, Connection* outputWire, ComponentInst** ancestry, int ancestry_length1, ComponentInst* componentInst) {
	return xor_component_state_construct (TYPE_XOR_COMPONENT_STATE, inputWires, inputWires_length1, outputWire, ancestry, ancestry_length1, componentInst);
}


static void xor_component_state_real_update (ComponentState* base) {
	XorComponentState * self;
	gboolean output;
	Connection** _tmp0_;
	gint _tmp0__length1;
	Connection* _tmp6_;
	gboolean _tmp7_;
	self = (XorComponentState*) base;
	output = FALSE;
...
static void xor_component_state_class_init (XorComponentStateClass * klass) {
	xor_component_state_parent_class = g_type_class_peek_parent (klass);
	COMPONENT_STATE_CLASS (klass)->finalize = xor_component_state_finalize;
	g_type_class_add_private (klass, sizeof (XorComponentStatePrivate));
	COMPONENT_STATE_CLASS (klass)->update = xor_component_state_real_update;
}


static void xor_component_state_instance_init (XorComponentState * self) {
	self->priv = XOR_COMPONENT_STATE_GET_PRIVATE (self);
}


static void xor_component_state_finalize (ComponentState* obj) {
	XorComponentState * self;
	self = XOR_COMPONENT_STATE (obj);
	self->priv->inputWires = (_vala_array_free (self->priv->inputWires, self->priv->inputWires_length1, (GDestroyNotify) connection_unref), NULL);
	_connection_unref0 (self->priv->outputWire);
	COMPONENT_STATE_CLASS (xor_component_state_parent_class)->finalize (obj);
}


GType xor_component_state_get_type (void) {
	static volatile gsize xor_component_state_type_id__volatile = 0;
	if (g_once_init_enter (&xor_component_state_type_id__volatile)) {
		static const GTypeInfo g_define_type_info = { sizeof (XorComponentStateClass), (GBaseInitFunc) NULL, (GBaseFinalizeFunc) NULL, (GClassInitFunc) xor_component_state_class_init, (GClassFinalizeFunc) NULL, NULL, sizeof (XorComponentState), 0, (GInstanceInitFunc) xor_component_state_instance_init, NULL };
		GType xor_component_state_type_id;
		xor_component_state_type_id = g_type_register_static (TYPE_COMPONENT_STATE, "XorComponentState", &g_define_type_info, 0);
		g_once_init_leave (&xor_component_state_type_id__volatile, xor_component_state_type_id);
	}
	return xor_component_state_type_id__volatile;
}
